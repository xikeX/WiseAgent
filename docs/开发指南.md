...
### 3.1. 智能体基本设计
  `WiseAgent` 设计之初面向的是多智能体系统，为了能够适应数量较多的智能体，需要尽可能的共享智能体之间的计算资源。\
  因此，`WiseAgent` 的设计原则是：智能体尽可能共享资源，智能体之间的差异以数据的相似表示。
  主要有几个特点
  - 共享行为：行为（Action）以单例模式存在，在需要时进行调用。
    $$共享行为(\text{ActionClass}) + 专有数据(\text{ActionData}) = 专有行为(\text{PersonalAction})$$
  - 按需加载：智能体在初始化的时候，只会加载智能体配置，不会载入行为类。在智能体初始化的时候，会根据智能体配置文件中的行为类名，进行行为类的加载。
  - 独立生命周期：智能体的生命周期以线程的方式存在，每个智能体都有自己独立的线程，独立的生命周期调度器。
  -  行为互不影响：利用线程和上下文管理器，能够让在编程时忽略多智能体的影响，从而能够专注于单个智能体的行为设计。
...
### 4.2. 自定义行为
本小节主要介绍如何自定义行为。在`WiseAgent`中，行为分为两种，一种为普通行为，另一种为计划行为。普通行为是智能体能够执行的最小单元，而计划行为是普通行为的组合，用于实现智能体的规划。
